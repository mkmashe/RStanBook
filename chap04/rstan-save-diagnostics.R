...
write.table(data.frame(summary(fit)$summary),
  file='./chap04/output/fit-summary.txt', sep='\t', quote=FALSE, col.names=NA)

ggmcmc(ggs(fit, inc_warmup=TRUE, stan_include_auxiliar=TRUE),
  file='./chap04/output/fit-traceplot.pdf', plot='traceplot')

ggmcmc(ggs(fit), file='./chap04/output/fit-ggmcmc.pdf')
# ggmcmc(ggs(fit), file='./chap04/output/fit-ggmcmc.pdf',
